...
{ 
  ...
  for(int i = 0; i < n; i++){
      ...
      while(counter < n){
        int minTimeIndex = 0;
        for(int i = 1; i < n; i++){
          if (rem_bt[i] < rem_bt[minTimeIndex]){
            minTimeIndex = i;
          }
        }
        
        rem_bt[minTimeIndex] = rem_bt[minTimeIndex] - 1;
        
        if (rem_bt[minTimeIndex] == 0){
          counter = counter + 1;
          completionTime = curTime + 1;
          plist[minTimeIndex].wt = completionTime - plist[minTimeIndex].bt;
          rem_bt[minTimeIndex] = 9999;
        }
      curTime = curTime + 1;
      }
